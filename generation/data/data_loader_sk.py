...
def get_transforms():
    ...
    return Compose([
        ...
        Lambdad(keys=["seg"], func=lambda path: load_tiff_with_tifffile(path)),
        
        # Ensure the data types are compatible with PyTorch tensors
        EnsureTyped(keys=["image", "mask", "seg"]),
        ...
        Lambdad(keys=["seg"], func=lambda x: x / 255.0),  
        ...
        Lambdad(keys=["seg"], func=lambda x: torch.tensor(x).permute(1, 3, 2, 0)),
    ])
...
def prepare_datasets():
    ...
    sk_files = [os.path.join("/data/wangfeiran/code/brainbow/datasets/skeleton_data/cropped_masks_80", f"neuron_{i}.tif") for i in range(train_number)]
    
    if len(image_files) != len(mask_files):
        raise ValueError("The number of images and masks do not match.")
    
    data_files = [{"image": img, "mask": msk, "seg":sk} for img, msk, sk in zip(image_files, mask_files, sk_files)]
