...
class DDPMTrainer:
    ...
    def initialize_unet(self):
        ...
        return DiffusionModelUNet(
            ...
            in_channels=1,                    # Match input channel size
            out_channels=1,                   # Output channel size; keep it same for DDPM
            num_res_blocks=2,                 # Number of residual blocks per level
            num_channels=(64, 128),           # Number of channels per level
            attention_levels=(False, True),   # Add attention at higher levels
            num_head_channels=(32, 64),       # Attention head channels for each level
            norm_num_groups=32,               # Number of groups for group normalization
            norm_eps=1e-5,                    # Epsilon for normalization stability
            resblock_updown=True,             # Use resblocks for up/downsampling
            with_conditioning=True,           # Enable conditioning for guided generation
            transformer_num_layers=1,         # Number of layers for transformer
            cross_attention_dim=128,          # Cross-attention context dimensionality
            num_class_embeds=None,            # Not using class conditioning
            upcast_attention=False,           # Avoid upcasting for efficiency
            use_flash_attention=False,        # Disable flash attention
            dropout_cattn=0.0                 # No dropout in cross-attention layers
        ).to(self.device)
    ...
    def calculate_metrics(self, synthetic_images, images):
        ...
        return mean_ssim, mean_mmd, mean_psnr
    ...
    def save_images(self, images, synthetic_images, masks, epoch):
        ...
        for idx in range(min(images.size(0), 5)):  # Save up to 5 samples
            ...
            synthetic_sample = synthetic_images[idx].permute(3, 0, 2, 1).cpu().numpy() * 255   # (Z, C, Y, X)
