...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        if mode == "concat":
            ...
            alpha = torch.nn.Parameter(torch.tensor(0.5))  # Learnable weight
            noisy_image = alpha * noisy_image + (1 - alpha) * condition
            condition = None
        diffusion_model = (
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            if mode == "concat":
                # CHANGED: concat to merge
                # model_input = torch.cat([image, conditioning], dim=1)
                alpha = torch.nn.Parameter(torch.tensor(0.5))  # Learnable weight
                model_input = alpha * image + (1 - alpha) * conditioning
